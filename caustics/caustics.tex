...
To be fair, no one algorithm is perfect: there is at least one lighting setup that cannot be efficiently modeled by using backward path tracing on its own.
...
See Fig. 11.
We shall work at implementing importance sampling with the backward path tracer, which is likely the easiest -- the most natural -- solution to the barrel light setup.
