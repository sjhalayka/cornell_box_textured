...
Although caustics can be produced using bidirectional or forward path tracing, thanks to the Helmholtz reciprocity, these caustics often suffer from convergence problems (e.g. low photon count).
...
The cosWeightedRandomHemisphereDirection function returns a pseudorandomly-generated direction vector in the same hemisphere as the surface normal.
}
\end{figure}
...
The stepAndOutputRNGFloat function returns a pseudorandom float in the range of 0.0 to 1.0.
}
\end{figure}
...
Note that the fog lights up because of the caustics.
