...
It's no wonder why it could be stated as a matter of fact in \cite{jensen} that photon mapping is hundreds of times faster than the path tracer.
This is obviously no longer the case, because 1) we use a sample count $s$ that is orders-of-magnitude smaller than what is used in \cite{jensen}, and 2) the backward path tracer is now accelerated by the graphics processing unit in Vulkan / Direct3D 12.
