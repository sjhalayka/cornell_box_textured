...
Rather than using a depth buffer though, one often uses a bounding volume hierarchy (BVH) \cite{wikipedia5} to determine eye ray / triangle intersection.

The backward path tracer is identical to the ray tracer, except that it also takes global illumination into account.
The backward path tracer often uses the exact same BVH setup as the ray tracer.
...
The simplicity of it all is the downfall of the technique.
}
\end{figure}
