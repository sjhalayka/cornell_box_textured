...
\item Appendix D: Failures of the backward path tracer, and the way forward
\end{itemize}
...
However, these caustics often suffer from convergence problems (e.g. low photon count).
These particular issues do not plague the backward path tracer.

Also, the geometry (triangles) of the light surface must be fully known in the case of bidirectional or forward path tracing.
...
\section{Failures of the backward path tracer}
To be fair, there is at least one lighting setup that cannot be efficiently modeled by using backward path tracing.
For instance, a light at the base of a long, thin hollow cylinder (e.g. a barrel light) is hard to get to when it comes to tracing a path from the eye.
See Fig. 11.
This particular issue does not plague the bidirectional or forward path tracer, or photon mapping.
In backward path tracing we avoid this particular issue by simply refraining from using barrel lights to begin with -- the lighting can be indirect, but not 
too indirect.
A more permanent solution to this particular issue would be to integrate the view space caustics into either the bidirectional or the forward path tracer.
